...
void view(){
	...
	employee *view_node=first_node,*reportee_node=first_node;
	
	unsigned char entered_id[max_size],buf_char,reportee_buffer[max_size];
	...
	if(view_node!=NULL){
		...
		/*else{
			...
			strcpy(reportee_buffer,view_node->name);
			strcat(reportee_buffer,"[");
			strcat(reportee_buffer,view_node->id);
			strcat(reportee_buffer,"]");
			printf("%20s\t:",first_node->reportees);
			while(reportee_node!=NULL){
				
				if(strcmp(reportee_node->manager,reportee_buffer))
					reportee_node=reportee_node->next;
				else{
					
					printf("%20s",reportee_node->name);
					printf("[%s]\n:",reportee_node->id);		
				}
				reportee_node=reportee_node->next;
			}
